...
def main() -> None:
    ...
    value_passed = (
        metrics["valueLogLoss"] < metrics["uniformValueLogLoss"]
        and metrics["valueBrier"] < metrics["uniformValueBrier"]
    )
    policy_passed = (
        metrics["policyCrossEntropy"] is not None
        and metrics["uniformPolicyCrossEntropy"] is not None
        and metrics["policyCrossEntropy"] < metrics["uniformPolicyCrossEntropy"]
    )
    metrics["promotionGate"] = {
        "valueBeatsUniform": bool(value_passed),
        "policyBeatsUniform": bool(policy_passed),
