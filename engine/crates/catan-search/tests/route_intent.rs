...
fn legal_road_intents_have_replayable_targets_and_finite_scores() {
    ...
    for action in state.legal_actions() {
        let Action::BuildRoad { edge } = action else {
            continue;
        };
        ...
        if let Some(target) = intent.target_vertex {
            ...
            assert!(
                next.board.vertices[usize::from(target)]
                    .adjacent_vertices
                    .iter()
...
fn best_authoritative_road_intent_survives_root_admission() {
    ...
    assert!(
        roots.iter().any(|(action, _)| match action {
            Action::BuildRoad { edge } =>
                (road_intent(&state, *edge, 0).ordering_score() - best_score).abs() < 1e-6,
            _ => false,
        }),
